...
class sheet {
    ...
    insert (data) {
        let self = this;
        return new Promise(function (resolve, reject) {
            self.collection.insert(data, function (err, item) {
    ...
    find (data) {
        let self = this;
        let option = arguments[1] ? arguments[1] : null;  //option = {limit:10,skip:10}
        return new Promise(function (resolve, reject) {
            let collections;
            collections = self.collection.find(data); //此处返回的是游标，用同样的变量会覆盖 上面获取的集合信息
            if (option) for (var i in option) collections = collections[i](option[i]); //在查询游标上加入条件
    ...
    remove (condition) {
        let self = this;
        return new Promise(function (resolve, reject) {
            self.collection.remove(condition, {}, function (err, results) {
                err ? reject(err) : resolve(results);
            });
        });
    }
